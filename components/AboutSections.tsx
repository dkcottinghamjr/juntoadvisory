...
function InfluenceGrid({ items }: { items: InfluenceItem[] }) {
  if (items.length === 0) {
    return <p className="text-sm text-ink/30 italic">Coming soon.</p>
  }
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
      {items.map((item) => (
        <a
          ...
          className="flex items-start gap-3 rounded-lg border border-border bg-white/60 px-4 py-3 hover:border-primary/30 hover:shadow-card transition-all group"
        >
          <svg
            className="w-5 h-5 flex-shrink-0 mt-0.5 text-primary/60 group-hover:text-primary transition-colors"
            fill="none"
            viewBox="0 0 24 24"
            strokeWidth={1.5}
            stroke="currentColor"
          ...
          <div className="min-w-0">
            <span className="text-sm font-medium text-ink/70 group-hover:text-ink transition-colors block truncate">
              {item.name}
            </span>
            {item.description && (
              <span className="text-xs text-ink/40 block line-clamp-2 leading-snug mt-0.5">
                {item.description}
              </span>
            )}
          </div>
      ...
      {sections.map((section, i) => {
        ...
        return (
          <FadeIn key={section.id} delay={i * 0.1}>
            <div id={section.id} className="border-b border-ink/15 scroll-mt-24">
              <button
                type="button"
              ...
              >
                <span className="font-display text-2xl md:text-3xl font-semibold text-ink group-hover:text-primary transition-colors">
                  {section.label}
                </span>
                <motion.span
                  animate={{ rotate: isOpen ? 180 : 0 }}
                  transition={{ duration: 0.2 }}
                  className="shrink-0 text-ink/50 group-hover:text-primary transition-colors"
                  aria-hidden="true"
                >
                  <svg
                    width="20"
